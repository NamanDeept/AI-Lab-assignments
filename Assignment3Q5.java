...
class Assignment3Q5 {
   ...
   static boolean checkPath(String path,int k){
       boolean result = true;
       //System.out.println(path);
       HashMap<String,Integer>friend = new HashMap();
      String []mypaths = path.split("\n");
      for(int i=0;i<mypaths.length;i++){
          String []myfriends = mypaths[i].split(" ");
          for(int j=1;j<myfriends.length;j++){
              if(!friend.containsKey(myfriends[j])){
                  friend.put(myfriends[j],1);
              }
              else {
                  int val = friend.get(myfriends[j]);
                  val++;
                  friend.put(myfriends[j],val);
              }
          }
      }
     //  System.out.println(friend);
      for(Map.Entry<String,Integer>x:friend.entrySet()){
         if(x.getValue()>k){
             result=false;
             break;
         } 
   ...
   static boolean checkFriend(ArrayList<String>friends,String [][]arr){
    ...
    static void computeSortedSwaps(String Source,int row,int col,ArrayList<String>friends,int k)throws NullPointerException
      { 
        int depth =0;
    ...
    public static void main(String args[]) throws Exception {
      ...
      computeSortedSwaps(source,m,n,frns,k);
      }
    }
